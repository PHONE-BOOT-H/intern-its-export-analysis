...
- **핵심 발견(검증됨)**: ADB 교통 낙찰의 60%는 안방 싸움(등록국=사업국)이고, 그걸 빼면 한국은 순수 수출 2위(중국 다음). 근데 ITS 니치는 대부분 도로공사라 MDB엔 ITS 기술 경쟁이 안 보인다. 양자 데이터(OECD CRS)에선 ITS가 실명으로 잡힌다(2022년 눈검증 확정 17건, 한국이 건수 최다 — 키워드 115건에서 오탐 걸러낸 결과) → 진짜 ITS 수출 싸움은 양자 채널(EDCF·BRI·JICA)에 있다. 핵심 산출물은 이걸 데이터로 증명하는 전략 브리프 2~3장.
